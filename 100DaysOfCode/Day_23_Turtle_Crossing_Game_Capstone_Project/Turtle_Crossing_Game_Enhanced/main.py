"""
This Turtle Crossing Game has been improved by adding the following functionalities:
    - Different cars sizes and more colors
    - Check car's safe distance
...
while game_is_on:
    ...
    # Check car's safe distance
    if len(car_manager.car_positions) >= 2:
        pos = -1
        last_car_position = car_manager.car_positions[pos]
        second_last_car_position = car_manager.car_positions[pos - 1]
        absolute_difference = abs(last_car_position - second_last_car_position)

        if absolute_difference < car_manager.safe_distance:
            car_manager.spawning = 15
